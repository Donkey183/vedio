...
public class VideoPlayerActivity extends MFBaseActivity
  ...
  public static final int UPDATE_QOSMESS = 9;
  ...
  private IMediaPlayer.OnPreparedListener mOnPreparedListener =
      new IMediaPlayer.OnPreparedListener() {
        @Override public void onPrepared(IMediaPlayer mp) {
          ...
          ksyMediaPlayer.setVideoScalingMode(
              KSYMediaPlayer.VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING);
          ...
          if (mQosThread != null && !mQosThread.isAlive()) mQosThread.start();
  ...
  private IMediaPlayer.OnVideoSizeChangedListener mOnVideoSizeChangeListener =
      new IMediaPlayer.OnVideoSizeChangedListener() {
        @Override public void onVideoSizeChanged(IMediaPlayer mp, int width, int height, int sarNum,
            int sarDen) {
    ...
    @Override public boolean onInfo(IMediaPlayer iMediaPlayer, int i, int i1) {
      switch (i) {
        ...
        case KSYMediaPlayer.MEDIA_INFO_SUGGEST_RELOAD:
          ...
          if (ksyMediaPlayer != null) ksyMediaPlayer.reload(mDataSource, false);
          break;
  ...
  public int setVideoProgress(int currentProgress) {
    ...
    if (!Constants.config.getVip_now().equals(Constants.CROWN)&&!Constants.config.getVip_now().equals(Constants.RED)) {
      length = ksyMediaPlayer.getDuration()*60;
    }else{
    ...
    if (time >= 0) {
      if (time / 1000 > Constants.config.getPlaytime()) {
        videoPlayEnd();
      }
  ...
  private void updateQosInfo(QosObject obj) {
    ...
    if (ksyMediaPlayer != null) {
      bits = ksyMediaPlayer.getDecodedDataSize() * 8 / (mPause ? mPauseStartTime
          - mPausedTime
          - mStartTime : System.currentTimeMillis() - mPausedTime - mStartTime);
  ...
  private final Callback mSurfaceCallback = new Callback() {
    ...
    @Override public void surfaceCreated(SurfaceHolder holder) {
      if (ksyMediaPlayer != null) ksyMediaPlayer.setDisplay(holder);
    }
  ...
  @Override public void onSurfaceTextureAvailable(SurfaceTexture surface, int i, int i1) {
    if (mSurfaceTexture == null) {
  ...
  public void savebitmap(Bitmap bitmap) {
    File appDir =
        new File(Environment.getExternalStorageDirectory(), "com.ksy.recordlib.demo.demo");
    if (!appDir.exists()) {
      appDir.mkdir();
    }
