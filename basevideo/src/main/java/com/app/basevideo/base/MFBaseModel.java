...
public abstract class MFBaseModel<T extends MFBaseActivity> {
     ...
     */
    public MFBaseModel(MFBaseActivity activityContext) {
        if (activityContext == null) {
            return;
        }
        this.mUniqueId = activityContext.getUniqueId();

        if (activityContext.isUseCacheData()) {
            loadModelDataFromCache();
        }
    }
     ...
     * 在每个具体的Model中调用{@link MFBaseModel#disPatchRequestSuccessMessage(Integer)}方法即可在发起请求的Activity收到请求数据
     */
